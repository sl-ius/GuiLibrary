...
Original by Sirius & Array

-------------------------------
Sius  | Designing + Programming + New Features
...
local LynxFieldQuality = {}

local LynxFieldLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across LynxField
			...
			SliderBackground = Color3.fromRGB(0, 88, 199),
			SliderProgress = Color3.fromRGB(0, 88, 199),
			SliderStroke = Color3.fromRGB(0, 88, 199),

			ToggleBackground = Color3.fromRGB(30, 30, 30),
			ToggleEnabled = Color3.fromRGB(0, 88, 199),
			ToggleDisabled = Color3.fromRGB(100, 100, 100),
			ToggleEnabledStroke = Color3.fromRGB(0, 88, 199),
		...
		Light = {
			TextFont = "Gotham",  -- Default will use the various font faces used across LynxField
...
local LynxField = game:GetObjects("rbxassetid://14109450820")[1]
...
if game["Run Service"]:IsStudio() then
	function gethui() return LynxField end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end
end

pcall(function()
	_G.LastRayField.Name = 'Old LynxField'
...
local ParentObject = function(Gui)
	...
	_G.LastRayField = LynxField
end
ParentObject(LynxField)

--Object Variables

local Camera = workspace.CurrentCamera
local Main = LynxField.Main
...
local InfoPrompt = LynxField.Info
...
local SelectedTheme = LynxFieldLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = LynxField.Theme[ThemeName]
	for _, obj in ipairs(LynxField:GetDescendants()) do
	...
	LynxField.Main.BackgroundColor3 = SelectedTheme.Background
	LynxField.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	LynxField.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	LynxField.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	LynxField.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	LynxField.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	LynxField.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if LynxFieldLibrary.Flags[FlagName] then
			spawn(function() 
				if LynxFieldLibrary.Flags[FlagName].Type == "ColorPicker" then
					LynxFieldLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if LynxFieldLibrary.Flags[FlagName].CurrentValue or LynxFieldLibrary.Flags[FlagName].CurrentKeybind or LynxFieldLibrary.Flags[FlagName].CurrentOption or LynxFieldLibrary.Flags[FlagName].Color ~= FlagValue then LynxFieldLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			LynxFieldLibrary:Notify({Title = "Flag Error", Content = "LynxField was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(LynxFieldLibrary.Flags) do
...
function LynxFieldLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= LynxFieldLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("LynxField | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if LynxField.Name == "LynxField" then
...
function Hide()
	...
	Debounce = true
	LynxFieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function LynxFieldLibrary:CreateWindow(Settings)
	LynxField.Enabled = false
	...
	pcall(function()
		...
		if not isfolder(LynxFieldLibrary.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(LynxFieldFolder.."/Discord Invites") then
			makefolder(LynxFieldFolder.."/Discord Invites")
		end
		if not isfile(LynxFieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(LynxFieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"LynxField RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(LynxFieldFolder.."/Key System") then
			makefolder(LynxFieldFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("LynxField | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(LynxFieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(LynxFieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function(EnterPressed)
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(LynxFieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						LynxFieldLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
	...
	--	qNotePrompt({
	--		Title = 'Preview',
	--		Description = 'This is a preview for the official LynxField forum post. Remember that things are subject to change.',
	...
	LynxFieldQuality.Window = {Tabs = {}}
	local Window = LynxFieldQuality.Window
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("LynxField | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateSection(SectionName,Display)
			...
			if Display then
				...
				SectionValue.Holder.Parent = LynxField.Holding
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function(enter)
				...
				if not Success then
					...
					print("LynxField | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			local function AddOption(Option,Selecteds)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("LynxField | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
					end
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						LynxFieldLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then  --Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("LynxField | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					LynxFieldLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("LynxField | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					LynxFieldLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					LynxFieldLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			local function UpdateSlider(X)
				...
				if SliderSettings.CurrentValue ~= NewValue then
					...
					if not Success then
						...
						print("LynxField | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					LynxFieldLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			...
			for name,info in pairs(PromptSettings.Actions) do
				...
				Button.Interact.MouseButton1Up:Connect(function()
					if not clicked then
						...
						if not Success then
							ClosePrompt()
							print("LynxField | "..info.Name.." Callback Error " ..tostring(Response))
						else
							ClosePrompt()
						end
					end
...
function LynxFieldLibrary:Destroy()
	LynxField:Destroy()
end
...
function LynxFieldLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				LynxFieldLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
task.delay(9, LynxFieldLibrary.LoadConfiguration, LynxFieldLibrary)
return LynxFieldLibrary
